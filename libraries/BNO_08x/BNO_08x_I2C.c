...
static HAL_StatusTypeDef sendPacket(const uint8_t channelNumber) {
    ...
    HAL_Delay(RESET_DELAY);  // Delay 100 microseconds before next I2C transmission
    return HAL_OK;
}
...
static HAL_StatusTypeDef receivePacket(void) {
    ...
    HAL_Delay(RESET_DELAY);  // Delay 100 microseconds before receiving the rest of the packet
    ...
    HAL_Delay(RESET_DELAY);  // Delay 100 microseconds after receiving the packet
    return HAL_OK;
}
